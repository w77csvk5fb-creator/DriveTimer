...
export function RiskBanner({ status }: RiskBannerProps) {
  ...
  return (
    <section
      className={`flex flex-col items-center gap-1 rounded-2xl border px-4 py-5 text-center ${presentation.bgClass} ${presentation.borderClass}`}
    >
      ...
      <p className="whitespace-nowrap text-xl font-bold tabular-nums text-on-surface">
        あと {formatDurationJa(status.freeTimeRemainingMs)} 走れます
      </p>
      <div className="mt-1 flex gap-4 whitespace-nowrap text-sm tabular-nums text-on-surface-muted">
